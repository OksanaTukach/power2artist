...
(function ($, undefined) {
	...
	$.extend(Datepicker.prototype, {
		...
		_generateHTML: function (t) {
			...
			D = isNaN(D) ? 0 : D;
			var C = this._get(t, "showWeek"), I = this._get(t, "dayNames");
			this._get(t, "dayNamesShort");
			var M = this._get(t, "dayNamesMin"), T = this._get(t, "monthNames"), P = this._get(t, "monthNamesShort"),
			...
			for (var A = this._getDefaultDate(t), O = "", E = 0; a[0] > E; E++) {
				var z = "";
				this.maxRows = 4;
				for (var W = 0; a[1] > W; W++) {
					var F = this._daylightSavingAdjust(new Date(f, p, t.selectedDay)), L = " ui-corner-all", R = "";
					if (h) {
					...
					R += '<div class="ui-datepicker-header ui-widget-header ui-helper-clearfix' + L + '">' + (/all|left/.test(L) && 0 == E ? i ? b : _ : "") + (/all|right/.test(L) && 0 == E ? i ? _ : b : "") + this._generateMonthYearHeader(t, p, f, u, d, E > 0 || W > 0, T, P) + '</div><table class="ui-datepicker-calendar"><thead>' + "<tr>";
					for (var Y = C ? '<th class="ui-datepicker-week-col">' + this._get(t, "weekHeader") + "</th>" : "", B = 0; 7 > B; B++) {
						var j = (B + D) % 7;
						Y += "<th" + ((B + D + 6) % 7 >= 5 ? ' class="ui-datepicker-week-end"' : "") + ">" + '<span title="' + I[j] + '">' + M[j] + "</span></th>"
					}
					R += Y + "</tr></thead><tbody>";
					var K = this._getDaysInMonth(f, p);
					f == t.selectedYear && p == t.selectedMonth && (t.selectedDay = Math.min(t.selectedDay, K));
					var q = (this._getFirstDayOfMonth(f, p) - D + 7) % 7, U = Math.ceil((q + K) / 7),
						V = h ? this.maxRows > U ? this.maxRows : U : U;
					this.maxRows = V;
					for (var X = this._daylightSavingAdjust(new Date(f, p, 1 - q)), G = 0; V > G; G++) {
						R += "<tr>";
						for (var Q = C ? '<td class="ui-datepicker-week-col">' + this._get(t, "calculateWeek")(X) + "</td>" : "", B = 0; 7 > B; B++) {
							var J = S ? S.apply(t.input ? t.input[0] : null, [X]) : [!0, ""], Z = X.getMonth() != p,
								te = Z && !N || !J[0] || u && u > X || d && X > d;
							Q += '<td class="' + ((B + D + 6) % 7 >= 5 ? " ui-datepicker-week-end" : "") + (Z ? " ui-datepicker-other-month" : "") + (X.getTime() == F.getTime() && p == t.selectedMonth && t._keyEvent || A.getTime() == X.getTime() && A.getTime() == F.getTime() ? " " + this._dayOverClass : "") + (te ? " " + this._unselectableClass + " ui-state-disabled" : "") + (Z && !H ? "" : " " + J[1] + (X.getTime() == c.getTime() ? " " + this._currentClass : "") + (X.getTime() == e.getTime() ? " ui-datepicker-today" : "")) + '"' + (Z && !H || !J[2] ? "" : ' title="' + J[2] + '"') + (te ? "" : ' data-handler="selectDay" data-event="click" data-month="' + X.getMonth() + '" data-year="' + X.getFullYear() + '"') + ">" + (Z && !H ? "&#xa0;" : te ? '<span class="ui-state-default">' + X.getDate() + "</span>" : '<a class="ui-state-default' + (X.getTime() == e.getTime() ? " ui-state-highlight" : "") + (X.getTime() == c.getTime() ? " ui-state-active" : "") + (Z ? " ui-priority-secondary" : "") + '" href="#">' + X.getDate() + "</a>") + "</td>", X.setDate(X.getDate() + 1), X = this._daylightSavingAdjust(X)
						}
						R += Q + "</tr>"
					}
					p++, p > 11 && (p = 0, f++), R += "</tbody></table>" + (h ? "</div>" + (a[0] > 0 && W == a[1] - 1 ? '<div class="ui-datepicker-row-break"></div>' : "") : ""), z += R
				}
				O += z
			}
			return O += x + ($.ui.ie6 && !t.inline ? '<iframe src="javascript:false;" class="ui-datepicker-cover" frameborder="0"></iframe>' : ""), t._keyEvent = !1, O
		},
